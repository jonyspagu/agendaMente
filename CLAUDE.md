...
- 🟡 Cutover real de Guada a Supabase — código listo y verificado (ver detalle arriba), commiteado, **sin desplegar a producción todavía**. Falta: coordinar la sesión real con ella (elegir contraseña, guardar código de recuperación) y, aparte, migrar su `historia` real.
- 🟡 Negocio (en curso, ya no pospuesto): marca, landing y Términos/Privacidad escritos. Dominio `agendamente.com.ar`/`www`/`app.agendamente.com.ar` conectados y en producción (Cloudflare Workers con assets, `sitio-agendamente`/`app-agendamente`, SSL automático). Resend configurado como SMTP custom de Supabase Auth (dominio verificado, plantillas de "Confirm signup"/"Reset password" en español), verificado con mails reales. Falta: Mercado Pago, revisión de abogado de los dos placeholders legales que quedan (Res. 424/2020 arrepentimiento; art. 12 transferencia internacional de datos) — este último queda pospuesto sin fecha por ahora.
