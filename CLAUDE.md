...
- 🔧 En construcción (2026-09-24): sincronización de Turnos → Google Calendar, **un solo sentido** (AgendaMente → Google). Ver plan completo en `.claude/plans/`. Decisión explícita: doble sentido (leer Google Calendar y bloquear horarios ocupados en la agenda) queda para una fase aparte, después de probar el sentido simple con clientas reales — no es un ajuste chico sobre lo de un sentido (agrega un concepto nuevo de "bloqueo de agenda" sin paciente, un cron de polling, y lógica anti-loop para no releer los propios eventos que mandamos a Google).
