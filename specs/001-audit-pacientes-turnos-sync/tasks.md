...
- [ ] T009 [US1] Reproducir en producción: crear un paciente de prueba con nombre `ZZZ_AUDITORIA_<timestamp>` desde el frontend en vivo y verificar en la hoja "Pacientes" si la columna `id` quedó completa o vacía (`quickstart.md` Paso 3.1-3.2) — **EN PAUSA: requiere acceso/acción directa sobre el Sheet de producción con datos clínicos reales; ver nota de alcance en informe-auditoria.md**
- [ ] T010 [US1] Si el paciente de T009 recibió un ID válido: crear un turno de prueba para ese paciente y verificar en "Turnos" + la agenda que el nombre del paciente se muestra correctamente vinculado (`quickstart.md` Paso 3.3) — **EN PAUSA, depende de T009**
- [ ] T011 [US1] Si el paciente de T009 NO recibió un ID válido: intentar igualmente crear un turno de prueba para ese paciente y documentar el comportamiento observado de la app (bloqueo total del dataset, turno huérfano, error visible, u otro) (`quickstart.md` Paso 3.4) — **EN PAUSA, depende de T009**
...
- [ ] T013 [US1] Eliminar del Sheet de producción cualquier fila de paciente/turno de prueba creada en T009-T011 (`quickstart.md` Paso 3.6) y confirmar que no queda residuo — **EN PAUSA, depende de T009-T011**
- [~] T014 [US1] Redactar el Hallazgo H1 (bug crítico) en `informe-auditoria.md`: causa raíz confirmada (o hipótesis explícitamente descartadas si no se pudo confirmar con certeza), ubicación exacta en frontend y/o backend, evidencia recopilada en T007-T012, y recomendación de corrección concreta — cumpliendo SC-001 — **H1 redactado con causa raíz de muy alta confianza vía análisis estático; falta solo la confirmación visual de un dato (texto exacto del header de "Pacientes"), que no requiere completar T009-T013**
