...
- [ ] T003 Ejecutar `npx @google/clasp login` para autenticar con la cuenta de Google dueña del proyecto Apps Script — **paso manual interactivo (OAuth en navegador), lo debe completar Jonatan** (ver `research.md` → Decisión 1)
- [ ] T004 Obtener el Script ID del proyecto Apps Script vinculado a la Web App actual (Project Settings, dentro del editor en script.google.com — no se puede derivar del `API_URL` de despliegue en `index.html:1284`)
- [ ] T005 Ejecutar `npx @google/clasp clone <SCRIPT_ID> --rootDir ./backend-appsscript` para traer el código backend real al repo
- [ ] T006 Listar los archivos `.gs` resultantes en `backend-appsscript/` e identificar cuáles contienen las funciones `create`/`update`/`delete` para las hojas "Pacientes" y "Turnos" — anotar los nombres de archivo/función en `informe-auditoria.md` (sección de referencia interna) para uso en las fases siguientes
...
- [ ] T007 [US1] En `backend-appsscript/`, revisar la función que maneja `sheet: "Pacientes", action: "create"`: documentar cómo genera y asigna el ID del paciente (autoincremento, `getLastRow()`, UUID, u otro) y si llama a `SpreadsheetApp.flush()` antes de devolver la respuesta — registrar hallazgo preliminar en `informe-auditoria.md`
- [ ] T008 [US1] Contrastar lo encontrado en T007 contra los puntos 1 y 2 de `contracts/apps-script-api.md` (garantía de que `res.id` corresponde a lo persistido; posible condición de carrera por falta de `flush()` antes de armar `res.pacientes`) y marcar cada punto como confirmado o refutado
- [ ] T009 [US1] Reproducir en producción: crear un paciente de prueba con nombre `ZZZ_AUDITORIA_<timestamp>` desde el frontend en vivo y verificar en la hoja "Pacientes" si la columna `id` quedó completa o vacía (`quickstart.md` Paso 3.1-3.2)
- [ ] T010 [US1] Si el paciente de T009 recibió un ID válido: crear un turno de prueba para ese paciente y verificar en "Turnos" + la agenda que el nombre del paciente se muestra correctamente vinculado (`quickstart.md` Paso 3.3)
- [ ] T011 [US1] Si el paciente de T009 NO recibió un ID válido: intentar igualmente crear un turno de prueba para ese paciente y documentar el comportamiento observado de la app (bloqueo total del dataset, turno huérfano, error visible, u otro) (`quickstart.md` Paso 3.4)
- [ ] T012 [US1] Analizar el mecanismo ya identificado por análisis estático en `index.html:1556-1562` (merge optimista de `agregarPaciente`, que acepta `nuevoId = Number(res.id)` sin verificar que exista en `res.pacientes`) y determinar si esto es la causa que propaga el bug hacia la UI, o si el origen real está solo del lado del backend confirmado en T007
- [ ] T013 [US1] Eliminar del Sheet de producción cualquier fila de paciente/turno de prueba creada en T009-T011 (`quickstart.md` Paso 3.6) y confirmar que no queda residuo
- [ ] T014 [US1] Redactar el Hallazgo H1 (bug crítico) en `informe-auditoria.md`: causa raíz confirmada (o hipótesis explícitamente descartadas si no se pudo confirmar con certeza), ubicación exacta en frontend y/o backend, evidencia recopilada en T007-T012, y recomendación de corrección concreta — cumpliendo SC-001

**Checkpoint**: US1 cerrado — causa raíz del bug crítico documentada en el informe, sin residuos de datos de prueba en producción.
