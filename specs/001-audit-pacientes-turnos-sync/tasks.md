...
- [X] T015 [P] [US2] En `backend-appsscript/`, revisar la función que maneja `sheet: "Turnos", action: "create"`: documentar si valida que `data.pacienteId` corresponda a un paciente existente antes de guardar, o si acepta cualquier valor sin validar (contrato punto 3 en `contracts/apps-script-api.md`) — **resultado: no valida nada (H3), es genérico para las 3 hojas**
- [X] T016 [P] [US2] En `backend-appsscript/`, revisar todas las funciones de escritura (create/update/delete de Pacientes, Turnos, Cobros) y verificar presencia y ubicación de `SpreadsheetApp.flush()` en cada una, siguiendo el Principio III de la constitución — **resultado: OK, `flush()` centralizado y correcto en los 3 puntos de escritura, sin gaps**
- [X] T017 [P] [US2] Analizar la asimetría de validación ya identificada entre `aplicarDataset` (`index.html:1387-1399`, bloquea la carga completa si algún `Paciente.id` no es numérico) y `normalizarTurno` (`index.html:1346-1349`, convierte `pacienteId` a `Number` sin validar `Number.isFinite`) — documentar como hallazgo de asimetría — **resultado: confirmado, y se extiende también a Cobros (H4)**
- [X] T018 [P] [US2] Analizar si `PacienteSearchSelect` (selector de paciente al crear un turno) puede ofrecer un paciente cuyo ID no está realmente confirmado en el servidor, dado que lee del estado local `pacientes` en memoria en vez de releer del backend en el momento de la selección — **resultado: confirmado, incluido en H4**
- [X] T019 [P] [US2] Verificar los puntos 3 y 4 de `contracts/apps-script-api.md` no cubiertos en US1 (validación de `pacienteId` en creación de turno; manejo de `ok: false` vs. `ok: true` con datos parcialmente inválidos) contra el código real en `backend-appsscript/` — **resultado: punto 3 confirmado sin validar (H3); punto 4 sin gaps, manejo de errores consistente**
- [X] T020 [US2] Redactar los Hallazgos H2..Hn en `informe-auditoria.md` (uno por cada punto de fragilidad de T015-T019), cada uno con severidad (crítico/medio/bajo), ubicación, causa raíz y recomendación — cumpliendo SC-002 — **CERRADO: H3-H8 redactados (incluye hallazgos extra encontrados fuera de lo previsto en tasks.md: condición de carrera en cobro automático H5, localStorage sin sincronizar H6, confirmación de Métricas H7 y de cobertura de Cobros H8)**

**Checkpoint**: ✅ US2 CERRADO — 6 hallazgos adicionales documentados (H3-H8: sin validación de FK, asimetría de validación extendida a Cobros, condición de carrera en cobro automático, localStorage sin sincronizar, confirmación de Métricas sin riesgo propio, confirmación de que Cobros ya queda cubierto por el fix de H1/H2) + confirmación positiva de que `flush()` no tiene gaps. Solo relevamiento — ningún fix aplicado, a pedido explícito del usuario.
